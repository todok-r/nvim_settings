...
function M.setup_telescope_keymaps()
  ...
  end, { desc = "Live Grep with dynamic -w" })

  local ft_to_extension = {
    ["javascript"] = "js",
    ["python"] = "py",
    ["typescript"] = "ts",
  }
  vim.keymap.set("n", "<leader>fs", function()
    builtin.live_grep({
      additional_args = function()
        ...
        local extension = ft_to_extension[ft] or ft
        local ret = { "--type=" .. extension }
        print(vim.inspect(ret))
        return ret
      end,
    })
  end, { desc = "Live Grep for filetype of buffer" })
end
